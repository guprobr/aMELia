...
QVector<MemoryRecord> MemoryManager::extractAutoMemories(const QString &userText) const
{
    ...
    const QRegularExpression wrcpRegex(QStringLiteral("\\bwrcp\\s+(\\d+\\.\\d+)\\b"), QRegularExpression::CaseInsensitiveOption);
    const QRegularExpressionMatch match = wrcpRegex.match(trimmed);
    if (match.hasMatch()) {
        MemoryRecord memory;
        memory.key = QStringLiteral("wrcp_version");
