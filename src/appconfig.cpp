...
AppConfig AppConfigLoader::load(const QString &path, QString *errorMessage)
{
    ...
    config.externalSearchTimeoutMs = readTimeoutValue(
        obj, QStringLiteral("externalSearchTimeoutMs"), config.externalSearchTimeoutMs, 2000);
    config.ollamaProbeTimeoutMs = readTimeoutValue(
        obj, QStringLiteral("ollamaProbeTimeoutMs"), config.ollamaProbeTimeoutMs, 2000);
    config.ollamaResponseHeadersTimeoutMs = readTimeoutValue(
        obj, QStringLiteral("ollamaResponseHeadersTimeoutMs"), config.ollamaResponseHeadersTimeoutMs, 5000);
    config.ollamaFirstTokenTimeoutMs = readTimeoutValue(
        obj, QStringLiteral("ollamaFirstTokenTimeoutMs"), config.ollamaFirstTokenTimeoutMs, 5000);
    config.ollamaInactivityTimeoutMs = readTimeoutValue(
        obj, QStringLiteral("ollamaInactivityTimeoutMs"), config.ollamaInactivityTimeoutMs, 5000);
    config.ollamaTotalTimeoutMs = readTimeoutValue(
        obj, QStringLiteral("ollamaTotalTimeoutMs"), config.ollamaTotalTimeoutMs, 5000, true);
