...
{
    QString requestedDataRoot = QDir::cleanPath(QDir::fromNativeSeparators(dataRoot.trimmed()));
    if (requestedDataRoot.isEmpty()) {
    ...
    QDir dir;
    if (!dir.mkpath(requestedDataRoot)) {
        if (errorMessage != nullptr) {
            *errorMessage = QStringLiteral("Failed to create directory: %1").arg(QDir::toNativeSeparators(requestedDataRoot));
        }
        return false;
    }

    m_dataRoot = canonicalRootPath(requestedDataRoot);
    if (m_dataRoot.isEmpty()) {
        m_dataRoot = requestedDataRoot;
    }

    const QString requestedKnowledgeRoot = knowledgeRoot.trimmed().isEmpty()
    ...
    m_knowledgeRoot = QDir(m_dataRoot).filePath(QStringLiteral("knowledge"));
    m_workspaceRoot = QDir(m_dataRoot).filePath(QStringLiteral("workspace"));
    m_runtimeWorkspaceRoot = QDir(m_workspaceRoot).filePath(QStringLiteral("runtime"));

    for (const QString &dirPath : {m_dataRoot, m_conversationsRoot, m_knowledgeRoot, m_workspaceRoot, m_runtimeWorkspaceRoot}) {
    ...
    const QString canonicalKnowledgeRoot = canonicalRootPath(m_knowledgeRoot);
    const QString canonicalWorkspaceRoot = canonicalRootPath(m_workspaceRoot);
    const QString canonicalRuntimeWorkspaceRoot = canonicalRootPath(m_runtimeWorkspaceRoot);
    const QString canonicalConversationsRoot = canonicalRootPath(m_conversationsRoot);
    if (!canonicalKnowledgeRoot.isEmpty()) {
        m_knowledgeRoot = canonicalKnowledgeRoot;
    }
    if (!canonicalWorkspaceRoot.isEmpty()) {
        m_workspaceRoot = canonicalWorkspaceRoot;
    }
    if (!canonicalRuntimeWorkspaceRoot.isEmpty()) {
        m_runtimeWorkspaceRoot = canonicalRuntimeWorkspaceRoot;
    }
    if (!canonicalConversationsRoot.isEmpty()) {
        m_conversationsRoot = canonicalConversationsRoot;
    }

    if (!isPathInsideRoot(m_conversationsRoot, m_dataRoot)
        || !isPathInsideRoot(m_knowledgeRoot, m_dataRoot)
        || !isPathInsideRoot(m_workspaceRoot, m_dataRoot)
        || !isPathInsideRoot(m_runtimeWorkspaceRoot, m_dataRoot)) {
        if (errorMessage != nullptr) {
            *errorMessage = QStringLiteral("Amelia storage paths must stay under the data root: %1")
                                .arg(QDir::toNativeSeparators(m_dataRoot));
        }
        return false;
    }

    const QString canonicalRequestedKnowledgeRoot = canonicalRootPath(requestedKnowledgeRoot);
    if (!canonicalRequestedKnowledgeRoot.isEmpty()
        && canonicalRequestedKnowledgeRoot != m_knowledgeRoot
        && errorMessage != nullptr) {
        *errorMessage = QStringLiteral("Configured knowledgeRoot is ignored outside Amelia's data root. Using %1.")
                            .arg(QDir::toNativeSeparators(m_knowledgeRoot));
    }
...
QString StorageManager::workspaceRoot() const
{
    return m_workspaceRoot;
}

QString StorageManager::runtimeWorkspaceRoot() const
{
    return m_runtimeWorkspaceRoot;
}

bool StorageManager::isPathInsideDataRoot(const QString &path) const
{
    return isPathInsideRoot(path, m_dataRoot);
}

bool StorageManager::isPathInsideKnowledgeRoot(const QString &path) const
{
    return isPathInsideRoot(path, m_knowledgeRoot);
}
...
QString StorageManager::canonicalRootPath(const QString &path) const
{
...
bool StorageManager::isPathInsideRoot(const QString &path, const QString &root) const
{
    const QString canonicalPath = canonicalRootPath(path);
    const QString canonicalRoot = canonicalRootPath(root);
