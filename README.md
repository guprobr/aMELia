...
- **Auto-continue for long answers**: Ollama's `num_predict` is intentionally left uncapped, so the only thing that previously cut an answer short was `num_ctx` filling up mid-response (reported by Ollama as `done_reason: "length"`). Amelia now detects that case and automatically sends a follow-up request asking the model to continue exactly where it left off — same grounding context, plus the tail of what's already been written — appending seamlessly to the same streamed answer instead of surfacing a hard-truncated one. Capped at 6 auto-continue rounds to avoid a runaway loop; a natural stop (`done_reason: "stop"`, e.g. the model's own `<END>` marker) ends the loop early as usual.
